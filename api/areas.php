...
function manejarDELETE($areaController) {
    $data = json_decode(file_get_contents('php://input'), true);
    ...
    $result = $areaController->eliminar($data['id']);
    ...
    if (isset($result['success']) && !$result['success']) {
        ...
        } elseif (isset($result['error']) && strpos($result['error'], 'no encontrada') !== false) {
